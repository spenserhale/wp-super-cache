...
if ( ! defined( 'PHP_VERSION_ID' ) ) {
	...
	define( 'PHP_VERSION_ID', (int) ($wpsc_php_version[0] * 10000 + $wpsc_php_version[1] * 100 + $wpsc_php_version[2]) );
...
function wp_cache_manager_error_checks() {
	...
	if ( $cache_enabled && ( !isset( $wp_cache_shutdown_gc ) || $wp_cache_shutdown_gc == 0 ) && function_exists( 'get_gc_flag' ) ) {
		$gc_flag = get_gc_flag();
		if ( ($dismiss_gc_warning == 0) && false == maybe_stop_gc( $gc_flag ) && false == wp_next_scheduled( 'wp_cache_gc' ) ) {
...
function wp_cache_manager_updates() {
	...
	if( isset( $_POST[ 'action' ] ) && $_POST[ 'action' ] == 'scupdates' ) {
		...
		if ( defined( 'WPSC_DISABLE_COMPRESSION' ) ) {
			...
			} else if ( $new_cache_compression != $cache_compression ) {
...
<?php
	...
	if ( ('preload' === $curr_tab) && true == $super_cache_enabled && ! defined( 'DISABLESUPERCACHEPRELOADING' ) ) {
			...
			if ( $cache_enabled ) {
				...
				if ( array_key_exists( 'action', $_POST ) && 'test' === $_POST['action'] && $valid_nonce ) {
					...
					if ( false == $cache_test_error ) {
						echo '<ul><li>' . sprintf( esc_html__( 'Page %d: %d (%s)', 'wp-super-cache' ), 1, (int) $page[1]['response']['code'], esc_attr( $page[1]['response']['message'] ) ) . '</li>';
						echo '<li>' . sprintf( esc_html__( 'Page %d: %d (%s)', 'wp-super-cache' ), 2, (int) $page[2]['response']['code'], esc_attr( $page[2]['response']['message'] ) ) . '</li></ul>';
					}

					if ( false == $cache_test_error && preg_match( '/(Cached page generated by WP-Super-Cache on) (\d*-\d*-\d* \d*:\d*:\d*)/', $page[1]['body'], $matches1 ) &&
							preg_match( '/(Cached page generated by WP-Super-Cache on) (\d*-\d*-\d* \d*:\d*:\d*)/', $page[2]['body'], $matches2 ) && $matches1[2] == $matches2[2]
					) {
...
function wpsc_plugins_tab() {
	...
	ob_start();
	if ( defined( 'WP_CACHE' ) && function_exists( 'do_cacheaction' ) ) {
...
&lt;!-- Cached page generated by WP-Super-Cache on " . date( "Y-m-d H:i:s" ) . " -->
...
function wp_cache_add_index_protection() {
	...
	if ( is_dir( $cache_path ) && false == is_file( "$cache_path/index.html" ) ) {
		...
		if ( (false == is_wp_error( $page )) && false == get_site_option( 'wp_super_cache_index_detected' )
...
function wp_cache_regenerate_cache_file_stats() {
	...
	$now = time();
	if (is_dir( $supercachedir ) && $dh = opendir( $supercachedir ) ) {
	...
	if ( $cache_compression ) {
		$sizes[ 'supercache' ][ 'cached' ]  = (int) ($sizes[ 'supercache' ][ 'cached' ] / 2);
		$sizes[ 'supercache' ][ 'expired' ] = (int) ($sizes[ 'supercache' ][ 'expired' ] / 2);
	}
...
function wpsc_dirsize($directory, $sizes) {
	...
	} else if ( is_file( $directory ) && strpos( $directory, 'meta-' . $file_prefix ) === false ) {
...
function wp_cache_admin_notice() {
	...
	if ( defined( 'WP_CACHE' ) && WP_CACHE == true && ( defined( 'ADVANCEDCACHEPROBLEM' ) || ( $cache_enabled && false == isset( $wp_cache_phase1_loaded ) ) ) && wp_cache_create_advanced_cache() ) {
...
function wp_cache_disable_plugin( $delete_config_file = true ) {
	...
	if ( @file_exists( WP_CONTENT_DIR . "/advanced-cache.php" ) && false == @unlink( WP_CONTENT_DIR . "/advanced-cache.php" ) ) {
	    $file_not_deleted[] = 'advanced-cache.php';
	}
	if ( $delete_config_file && @file_exists( WP_CONTENT_DIR . "/wp-cache-config.php" ) && false == unlink( WP_CONTENT_DIR . "/wp-cache-config.php" ) ) {
