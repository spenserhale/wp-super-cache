...
function wp_cache_serve_cache_file() {
	...
	if (
		...
		if ( false == file_exists( $file ) ) {
			...
			return false;
		} elseif ( ! empty( $wpsc_save_headers ) ) {
		...
		if ( $wp_cache_request_uri == $wp_cache_home_path || ( $wp_cache_slash_check && substr( $wp_cache_request_uri, -1 ) == '/' ) || ( $wp_cache_slash_check == 0 && substr( $wp_cache_request_uri, -1 ) != '/' ) ) {
			...
			if ( $wp_cache_gzip_encoding ) {
				...
				header( 'Content-Length: ' . $size );
			} elseif ( ! empty( $wpsc_served_header ) ) {
...
function get_all_supercache_filenames( $dir = '' ) {
	...
	if ( $dir != '' && ! empty( $wp_cache_mobile_enabled ) ) {
...
function wp_cache_phase2() {
	global $super_cache_enabled, $cache_rebuild_files, $cache_enabled, $wp_cache_gmt_offset, $wp_cache_blog_charset;
...
function wp_cache_mutex_init() {
	...
	if ( defined( 'WPSC_DISABLE_LOCKING' ) || ! empty( $wp_cache_mutex_disabled ) )
		return true;
...
function wp_cache_get_ob(&$buffer) {
	...
	if ( $buffer == '' ) {
		$new_cache = false;
		if ( ! empty( $GLOBALS[ 'wp_super_cache_debug' ] ) ) {
	...
	if ( $added_cache && ! empty( $wp_supercache_cache_list ) ) {
...
function wp_cache_shutdown_callback() {
	global $meta_file, $new_cache, $wp_cache_meta, $blog_id, $wp_cache_gzip_encoding, $supercacheonly;
...
function wp_cache_post_change( $post_id ) {
	global $file_prefix, $blog_id, $super_cache_enabled, $blog_cache_dir, $wp_cache_refresh_single_only;
	...
	if (
		! empty( $wp_cache_refresh_single_only ) &&
		(
